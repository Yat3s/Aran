...
def process_command(content, from_user_id, from_user_name):
    ...
    if u'你的主人' in content:
        chatcore.send(u'我的主人是美丽可爱的李小萌同学~嘻嘻[Hey]', from_user_id)
        return True
    ...
    if u'色情网站' in content or u'黄色网站' in content:
        chatcore.send(u'你可以看看1024，tumblr，91，草榴，等等，如果想知道更多，可以联系我主人哦嘻嘻ლ(＾ω＾ლ)', from_user_id)
        return True
    ...
    if u'小萌' in content:
        chatcore.send(u'小萌是我可爱的主人大大~Mua', from_user_id)
        return True
    ...
    if u'美女图' in content:
        if isAdmin:
            ...
            chatcore.send(u'这个指令主人说了不能给别人用哦TT', from_user_id)
        return True
    ...
    if u'Search' in content:
        if isAdmin:
            ...
            chatcore.send(u'这个指令主人说了不能给别人用哦TT', from_user_id)
        return True
    if u'叫主人' in content or u'叫爸爸' in content:
        if isAdmin:
            chatcore.send(u'萌主人好~，主人有什么吩咐吗？', from_user_id)
        else:
            chatcore.send(u'这个指令主人说了不能给别人用哦TT', from_user_id)
        return True
    ...
    if u'开放权限' in content:
        if isAdmin:
            OPEN_AUTH = True
            chatcore.send(u"萌主人，权限已经开放啦~", from_user_id)
        else:
            chatcore.send(u'这个指令主人说了不能给别人用哦TT', from_user_id)
        return True
    ...
    if u'关闭权限' in content:
        if isAdmin:
            OPEN_AUTH = False
            chatcore.send(u"萌主人，权限已经关闭了~", from_user_id)
        else:
            chatcore.send(u'这个指令主人说了不能给别人用哦TT', from_user_id)
        return True
    return False
